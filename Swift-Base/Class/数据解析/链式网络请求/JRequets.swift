...
extension NetworkKit{
    ...
    func request() -> Void {
        ...
        if let URLString = url {
            ...
            var head: String = "Bearer "
            head.append(UserDefaults.standard.value(forKey: "Token") as! String)
            
            
            let header: HTTPHeaders = ["Authorization" : head]
            
            //dataRequest =  Alamofire.request(URLString, method: method, parameters: params, headers: header)
            dataRequest =  Alamofire.request(URLString, method: method, parameters: params, headers: header)
